...
namespace AppBancoDigital.Service
{
    public class DataService
    {
        ...
        protected static async Task<string> PostDataToService(string json_object, string rota)
        {
            ...
            using (HttpClient client = new HttpClient())
            {
                HttpResponseMessage response = await client.PostAsync(uri, new StringContent(json_object, Encoding.UTF8, "application/json"));
                if (response.IsSuccessStatusCode)
                {
                    json_response = response.Content.ReadAsStringAsync().Result;
                }
        ...
        protected static string DecodeServerError(System.Net.HttpStatusCode status_code) 
        {
            string msg_erro;
